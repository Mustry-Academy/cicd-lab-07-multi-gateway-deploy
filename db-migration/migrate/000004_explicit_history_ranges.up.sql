CREATE FUNCTION oat_demo.snapshot_range(p_start_ms bigint,p_end_ms bigint,p_line integer DEFAULT 0,p_batch text DEFAULT '')
...
BEGIN
 ...
 finish := least(to_timestamp(p_end_ms/1000.0),wall);
 begin_at := to_timestamp(p_start_ms/1000.0);
 span := finish-begin_at;
 IF span<=interval '0 seconds' OR span>interval '90 days' THEN RAISE EXCEPTION 'Invalid history range'; END IF;
 bucket := make_interval(secs=>greatest(60,ceil(extract(epoch FROM span)/300/60)*60)::integer);
 ...
 SELECT sum(s.total_kg) total, sum(s.good_kg) good, sum(s.energy_kwh) energy,
  sum(l.nominal_kg_min*s.seconds/60) planned, count(*) n,
  count(*) FILTER(WHERE s.state NOT IN ('Stopped','Changeover')) running,
  sum(l.nominal_kg_min*s.seconds/60) FILTER(WHERE s.state NOT IN ('Stopped','Changeover')) running_plan,
  avg(s.moisture) moisture, avg(s.temperature) temperature
 INTO totals FROM oat_demo.range_points(begin_at,finish,p_line) s JOIN oat_demo.line l ON s.line_id=l.id
 WHERE true AND (p_line=0 OR s.line_id=p_line);
 IF totals.n=0 THEN
 ...
 RETURN jsonb_build_object('ready',true,'simulated',true,'message',CASE WHEN NOT coalesce((health_state->>'ok')::boolean,false) THEN 'Data is stale or inconsistent. The simulator needs attention.' ELSE 'Simulation running. Updates every minute.' END,
  'stale',NOT coalesce((health_state->>'ok')::boolean,false),'scenario','history',
...
CREATE FUNCTION oat_demo.batches_range(p_start_ms bigint,p_end_ms bigint,p_line integer DEFAULT 0)
RETURNS jsonb LANGUAGE plpgsql STABLE AS $$
...
CREATE FUNCTION oat_demo.batch_detail(p_reference text)
RETURNS jsonb LANGUAGE plpgsql STABLE AS $$
